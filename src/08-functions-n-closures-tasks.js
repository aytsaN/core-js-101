...
function getPolynom(...args) {
  return (x) => [...args].reverse().reduce((prev, cur, i) => prev + cur * x ** i);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const logOutput = `${func.name}(${args.map((el) => JSON.stringify(el))})`;
    logFunc(`${logOutput} starts`);
    const res = func(...args);
    logFunc(`${logOutput} ends`);
    return res;
  };
}
